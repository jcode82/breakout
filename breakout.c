...
int main(void)
{
    ...
    //waiting for mouse click before starting game
    // waitForClick();
    //listening for mouse click
      // GEvent event = getNextEvent(MOUSE_EVENT);
      // if (getEventType (event) = MOUSE_CLICK)
      // {
...
//////////////////////////////////////////////////////////////////
        ...
        if (points == 25)
        {
            GLabel win = newGLabel("Your a Camacho!");
...
GOval initBall(GWindow window)
{
    // TODO
      ...
      //double velocity1 = 2.0;
      //cdouble velocity2 = 2.0;

      /*while (true)
      {
          move(oval, velocity1, velocity2);

          if (getX(oval) + getWidth(oval) >= getWidth(window))
          {
              velocity1 = -velocity1;
          }
          else if (getX(oval) <= 0)
          ...
          if (getY(oval) + getHeight(oval) >= getHeight(window))
          {
              velocity2 = -velocity2;
          }
          else if (getY(oval) <= 0)
...
GRect initPaddle(GWindow window)
{
    ...
    /*while (true)
    {
        ...
        if (event != NULL)
        {
            if (getEventType(event) == MOUSE_MOVED)
            {
                double x = getX(event) - getWidth(rect) / 2;
                double y = getX(event) - getWidth(rect);
    ...
    while (true)
    {
        move(rect, velocity, 0);

        if (getX(rect) + getWidth(rect) >= getWidth(window))
        {
            velocity = -velocity;
        }
        else if (getX(rect) <= 0)
